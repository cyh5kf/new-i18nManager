...
function createColumn(title,key,render,ext){
    var obj =  {
        title: title,
        dataIndex: key,
        key: key,
...
export default class I18nMainView extends React.Component {
    ...
    expandedRowRender=(record)=>{
        ...
        var languageCodes = store.languageCodes || [];
        return (
            <ul className="langDetail">
                {languageCodes.map(function(langCode){
                    var key = record.id + "_" +langCode;
                    return (
                        <li key={key}>
                            <span className="langName">{languageMap[langCode]}</span>
                            <span className="langValue">{record["value_"+langCode]}</span>
                        </li>
                    )
                })}
            </ul>);
